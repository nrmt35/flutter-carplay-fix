...
class FlutterCarPlayController {
  ...
  /// Buttons currently shown on the shared Now Playing screen, set via
  /// [FlutterCarplay.updateNowPlayingButtons].
  static List<CPNowPlayingButton> currentNowPlayingButtons = [];

  /// Whether the shared Now Playing template is currently the visible top
  /// template. Kept in sync from the native side (see
  /// [FCPChannelTypes.onNowPlayingActiveChange]) so callers can avoid
  /// rebuilding the root template while it is on screen.
  static bool isNowPlayingActive = false;
  ...
  void processFCPNowPlayingButtonPressed(String elementId) {
    for (var b in currentNowPlayingButtons) {
      if (b.uniqueId == elementId) {
        b.onPress?.call();
        return;
      }
