...
final class FCPListImageRowItem {
  ...
  private var placeholderKind: String?
  ...
  init(obj: [String: Any]) {
    ...
    self.placeholderKind = obj["placeholderKind"] as? String
  ...
  var get: CPListTemplateItem {
    ...
    if #available(iOS 26.0, *), let firstElement = elements.first {
      ...
      let placeholderImages = Array(
        repeating: makeSafeArtworkPlaceholder(kind: placeholderKind), count: gridImages.count)
      ...
      for (index, imagePath) in gridImages.prefix(maxCount).enumerated() {
        ...
        loadUIImageAsync(from: imageSource, placeholderKind: placeholderKind) { uiImage in
