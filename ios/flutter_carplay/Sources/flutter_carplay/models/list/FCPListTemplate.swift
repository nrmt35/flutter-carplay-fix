...
class FCPListTemplate {
  ...
  var get: CPTemplate {
    ...
    // Only assign the empty-view variants when non-empty. CarPlay shows its
    // built-in loading spinner for a list with no sections *only* when these
    // are left unset; assigning an empty array configures a (blank) empty view
    // instead, which suppresses the spinner. Leaving them nil lets the spinner
    // appear while content is being fetched.
    if !emptyViewTitleVariants.isEmpty {
      listTemplate.emptyViewTitleVariants = emptyViewTitleVariants
    }
    if !emptyViewSubtitleVariants.isEmpty {
